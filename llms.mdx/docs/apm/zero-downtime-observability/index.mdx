...
Out-of-the-Box Value [#out-of-the-box-value]
...
1. Universal RED Metrics [#1-universal-red-metrics]
...
2. Auto-Distributed Tracing [#2-auto-distributed-tracing]
...
3. Dynamic Service Inventory & Metadata [#3-dynamic-service-inventory--metadata]
...
4. Granular Network Observability [#4-granular-network-observability]
...
The KloudMate APM Edge vs. Traditional Telemetry [#the-kloudmate-apm-edge-vs-traditional-telemetry]
...
Deploying APM Strategically [#deploying-apm-strategically]
...
**Enrich Where Necessary:** Once eBPF has illuminated the vast majority of your distributed architecture, strategically deploy [Manual Instrumentation](/docs/apm/manual-instrumentation) SDKs into the specific handful of applications requiring bespoke business logic mapping (such as tracing a specific User ID or capturing unique transaction states).
...
Running the Agent with eBPF in Kubernetes [#running-the-agent-with-ebpf-in-kubernetes]

When installing via the official KloudMate Helm Chart (as outlined in the [Installation Guide](/docs/installation#kubernetes-installation)), the eBPF support is automatically configured with necessary volume mounts and permissions.
